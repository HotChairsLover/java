...
public class RoadController {
    ...
    public static void main(String[] args) {
        ...
        int carsCount = scanner.nextInt(); // int

        for (int i = 0; i < carsCount; i++) { // цикл + int
     ...
     */
    private static int calculatePrice(Car car) { // int
        int carHeight = car.height; // int
        int price = 0; // int
        if (carHeight > controllerMaxHeight) {
            blockWay("высота вашего ТС превышает высоту пропускного пункта!");
            return -1;
        } else if (carHeight > passengerCarMaxHeight) {
            double weight = car.weight; // double
            //Грузовой автомобиль
